...
cleanup() {
  ...
  kubectl delete -f "$REPO/testing/scripts/manifests/dependency-e2e/db-test-prod.yaml" --ignore-not-found --wait=false
}
...
ensure_shadowtest_crd() {
  ...
  make -C "$REPO/pipeline/monarch" manifests
  ...
  local manifest="$REPO/testing/scripts/manifests/dependency-e2e/shadowtest-deps.yaml"
  local i
...
if [[ "$SKIP_BUILD" != "1" ]]; then
  ...
  make -C "$REPO/testing/example-apps/db-test-app" docker-build DB_TEST_IMG="$DB_TEST_IMG"
fi
...
if [[ "$SKIP_MONARCH_DEPLOY" != "1" ]]; then
  ...
  kubectl get deploy -n monarch-system monarch-controller-manager >/dev/null 2>&1 || {
    log_fail "Monarch not deployed — unset SKIP_MONARCH_DEPLOY or run: ./testing/scripts/e2e-reset-kind.sh"
    exit 1
  }
fi
...
kubectl apply -f "$REPO/testing/scripts/manifests/dependency-e2e/db-test-prod.yaml"

ST_MANIFEST="$REPO/testing/scripts/manifests/dependency-e2e/shadowtest-deps.yaml"
...
if [[ "${avail:-0}" -lt 1 ]]; then
  ...
  echo "    Common fixes:" >&2
  echo "      - Rebuild Monarch: make -C pipeline/monarch docker-build IMG=${MONARCH_IMG} && kind load docker-image ${MONARCH_IMG}" >&2
...
if ! kubectl get deploy redis-control-a -n "$SHADOW_NS" >/dev/null 2>&1; then
  ...
  echo "      make -C pipeline/monarch docker-build IMG=${MONARCH_IMG} && kind load docker-image ${MONARCH_IMG} --name ${KIND_CLUSTER}" >&2
  echo "      make -C pipeline/monarch deploy IMG=${MONARCH_IMG}" >&2

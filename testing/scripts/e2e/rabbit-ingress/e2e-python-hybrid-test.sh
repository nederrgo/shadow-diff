...
# Minikube only. Pixie HTTP seed auto-enabled when the pl namespace exists.
#
...
echo "==> Python hybrid E2E (minikube pixie=${USE_PIXIE}; Mongo OTLP + HTTP replay + RMQ Firehose)"
...
for role in control-a control-b candidate; do
  ...
  if ! kubectl logs -n "$SHADOW_NS" "$pod" -c app --tail=120 2>/dev/null | grep -q "http egress.*status=200"; then
    log_fail "${role} missing http egress status=200 (transparent proxy miss?)"

...
# ponytail: runs all exports for one rule; called in a subshell by reconcile_rules
_export_rule() {
  local rules_json="$1" i="$2"
  local rule_json name ns active
...
reconcile_rules() {
  local rules_json rule_count i=0
  local -a pids=()
...
trap 'kill $(jobs -p) 2>/dev/null; echo "==> pixie-stream-bridge stopping"; exit 0' INT TERM

...
wait_pixie_vizier_pem() {
  ...
  while [[ "$i" -lt "$max_wait" ]]; do
    if kubectl get pods -n pl -l name=vizier-pem --no-headers 2>/dev/null \
        | awk '$3=="Running"{ok=1} END{exit !ok}'; then
...
# curl_prod_service PROD_NS TRACEPARENT PATH [SVC]
# Sends an HTTP request to the prod service with W3C traceparent header.
curl_prod_service() {
  local prod_ns="$1" traceparent="$2" path="${3:-/}" svc="${4:-my-prod-app}"
  local host name
  host=$(prod_service_host "$prod_ns" "$svc")
  name="prod-hit-${traceparent//[^a-zA-Z0-9]/-}"
  name="${name:0:50}"
...
assert_shadow_roles_saw_trace() {
  ...
  for role in control-a control-b candidate; do
    ...
    if [[ -z "$pod" ]]; then
      echo "ERROR: no shadow app pod for role ${role}" >&2; return 1
    fi

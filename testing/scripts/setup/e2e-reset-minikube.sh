...
# shellcheck source=testing/scripts/helpers/e2e-reset-deploy.sh
source "$REPO/testing/scripts/helpers/e2e-reset-deploy.sh"
ensure_go_path
...
if [[ "${SETUP_PIXIE:-1}" -eq 1 && "${SKIP_PIXIE:-0}" -eq 0 ]]; then
  ...
  chmod +x "$REPO/testing/scripts/setup/setup-local-pixie.sh"
  "$REPO/testing/scripts/setup/setup-local-pixie.sh" --skip-minikube-start
fi

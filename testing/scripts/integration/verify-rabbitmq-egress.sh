...
SPAN_HEX="${SPAN_HEX:-$(openssl rand -hex 8)}"
...
if [[ -z "$SHADOW_NS" ]]; then
  log_fail "ShadowTest ${SHADOWTEST} not ready — run ./testing/scripts/e2e-rabbitmq-egress-test.sh first"
  exit 1
fi
...
kubectl exec -n default deploy/rmq-prod-broker -- sh -c "
  ...
  rabbitmqadmin publish exchange=${PROD_EXCHANGE} routing_key=${PROD_ROUTING_KEY} \
    payload='{\"e2e\":\"rmq-egress-verify\"}' properties='{\"headers\":{\"traceparent\":\"${TRACE_TP}\"}}'
"

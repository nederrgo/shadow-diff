...
#   - Minikube cluster with Beru + Monarch + mongo-test ShadowTest deployed
...
kubectl get deploy -n "$BERU_NS" beru >/dev/null 2>&1 || {
  log_fail "Beru not deployed in ${BERU_NS} — run ./testing/scripts/e2e-reset-minikube.sh first"
  exit 1
}

...
if ! minikube -p "${MINIKUBE_PROFILE:-minikube}" status --format='{{.Host}}' 2>/dev/null | grep -qi running; then
  log_fail "minikube profile ${MINIKUBE_PROFILE:-minikube} is not running — start with: ./testing/scripts/e2e-reset-minikube.sh"
  exit 1
fi
...
kubectl wait --for=condition=Available deployment/nodejs-prod-worker -n default --timeout=180s
...
for i in $(seq 1 45); do
  PROD_LOGS="$(kubectl logs -n default deploy/nodejs-prod-worker --tail=120 2>/dev/null || true)"

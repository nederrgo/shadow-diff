...
- Images built on host docker and loaded via `kind load`: `monarch:dev`, `igris-rabbitmq:dev`, `egress-relay-rabbitmq:dev`, and the suite worker images
- Monarch operator with **`MONARCH_MODE=dev`** (set by `e2e-reset-kind.sh` and bats `ensure_platform_ready`) so helper images resolve to `:dev` tags without CR image overrides
...
kind load docker-image monarch:dev --name shadow-diff

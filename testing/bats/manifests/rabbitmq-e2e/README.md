...
- Kind cluster with Beru and Monarch (`./testing/bats/e2e-reset-kind.sh`)
- Images built/loaded: `monarch:dev`, `igris-rabbitmq:dev`, `egress-relay-rabbitmq:dev`, `rmq-test-worker:dev`, `recorder:dev` (when `spec.recordAndReplay` is set)
...
```bash
./testing/bats/e2e-reset-kind.sh --no-reset
./testing/bats/e2e-rabbitmq-test.sh
```

Or:

```bash
./testing/bats/e2e-reset-kind.sh --run-rabbitmq-test
```

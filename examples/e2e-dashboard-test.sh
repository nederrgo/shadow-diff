...
#
# Prerequisites:
#   - Kind cluster with Beru deployed (./scripts/e2e-reset-kind.sh)
...
#
# Usage:
#   ./examples/e2e-dashboard-test.sh
#   BERU_IMG=beru:dev ./examples/e2e-dashboard-test.sh
#   SKIP_BERU_BUILD=1 ./examples/e2e-dashboard-test.sh
#
...
kubectl get deploy -n "$BERU_NS" beru >/dev/null 2>&1 || {
  log_fail "Beru not deployed in ${BERU_NS} — run: ./scripts/e2e-reset-kind.sh"
  exit 1
}

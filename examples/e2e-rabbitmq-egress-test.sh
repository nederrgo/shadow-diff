...
if [[ "$SKIP_BUILD" != "1" ]]; then
  ...
  make -C "$REPO/examples/rmq-test-worker" docker-build RMQ_TEST_WORKER_IMG="$RMQ_WORKER_IMG"
  echo "==> Build igris-rabbitmq image"
  make -C "$REPO/igris-rabbitmq" docker-build IGRIS_RABBITMQ_IMG="$IGRIS_RABBITMQ_IMG"
  echo "==> Build egress-relay-rabbitmq image"
  make -C "$REPO/egress-relay-rabbitmq" docker-build EGRESS_RELAY_RABBITMQ_IMG="$EGRESS_RELAY_RABBITMQ_IMG"
fi
...
kubectl apply -f "$REPO/tests/rabbitmq-egress-e2e/prod-target-egress.yaml"
...
kubectl apply -f "$REPO/tests/rabbitmq-egress-e2e/shadowtest-rmq-egress.yaml"

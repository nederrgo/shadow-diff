...
#   - ./scripts/e2e-reset-kind.sh  OR  ./examples/e2e-mongo-egress-test.sh
#
# Usage:
#   ./examples/verify-mongo-egress.sh
#   TRACE_ID=my-test-123 ./examples/verify-mongo-egress.sh
#   SHADOWTEST_NS=default WAIT_SECS=45 ./examples/verify-mongo-egress.sh
set -euo pipefail
...
kubectl get deploy -n "$BERU_NS" beru >/dev/null 2>&1 || {
  log_fail "Beru not deployed in ${BERU_NS} — run ./scripts/e2e-reset-kind.sh first"
  exit 1
}

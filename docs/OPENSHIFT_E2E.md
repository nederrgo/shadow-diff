...
This guide deploys Monarch, Beru, and a `ShadowTest` on **Red Hat OpenShift 4.x** and runs the same egress E2E flow as [`examples/e2e-egress-test.sh`](../examples/e2e-egress-test.sh).
...
# Build (uses scripts/lib/docker.sh to avoid WSL credential-helper issues)
...
sed "s/namespace: default/namespace: ${OC_PROJECT}/" examples/e2e-prod-app.yaml | oc apply -f -
...
chmod +x examples/e2e-egress-test.sh
...
# 3. Install stack
make -C monarch install && make -C monarch deploy IMG=$MONARCH_IMG
oc apply -f beru/deploy/ && oc set image deployment/beru beru=$BERU_IMG -n beru-system
...
- Kind E2E reset: [`scripts/e2e-reset-kind.sh`](../scripts/e2e-reset-kind.sh)
- Egress test script: [`examples/e2e-egress-test.sh`](../examples/e2e-egress-test.sh)

...
```bash
./testing/scripts/e2e-reset-kind.sh --run-test    # deploy stack + pipeline
./testing/scripts/e2e-pipeline-test.sh           # traffic only (uses node hostIP for Siphon API)
```
...
| [testing/scripts/manifests/e2e-shadowtest.yaml](testing/scripts/manifests/e2e-shadowtest.yaml) | Reference ShadowTest with correct ports |

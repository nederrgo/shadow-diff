...
This document describes how **Monarch**, **Igris**, **Beru**, and **Siphon** fit together in the monorepo. For Monarch directory layout and development workflow, see [monarch/REPO_OVERVIEW.md](pipeline/monarch/REPO_OVERVIEW.md).
...
| Path | Module | Role |
|------|--------|------|
| [`monarch/`](pipeline/monarch/) | `github.com/shadow-diff/monarch` | Kubernetes operator — `ShadowTest` CRD, shadow namespace, Deployments, Envoy sidecar config |
| [`igris/`](igris/) | `github.com/shadow-diff/igris` | HTTP multicaster — L7 fan-out to control-a, control-b, candidate |
| [`beru/`](pipeline/beru/) | `github.com/shadow-diff/beru` | gRPC differ — ingest, correlation, diff-of-diffs |
| [`siphon/`](pipeline/siphon/) | `github.com/shadow-diff/siphon` | AF_PACKET capture agent — kernel classic BPF filter, TCP reassembly, HTTP replay to Igris |
...
- Apply the Siphon DaemonSet manifest directly in Kind E2E — Monarch creates/patches the DaemonSet; only **`pipeline/siphon/deploy/rbac.yaml`** (namespace + ServiceAccount) is applied upfront.
...
**Details:** [monarch/REPO_OVERVIEW.md](pipeline/monarch/REPO_OVERVIEW.md), [monarch/DEPLOYMENT.md](pipeline/monarch/DEPLOYMENT.md), [`testing/scripts/lib/siphon-config.sh`](testing/scripts/lib/siphon-config.sh) (E2E wait helpers).
...
**Code:** [`igris/internal/core/`](igris/internal/core/), [`igris/internal/driver/`](igris/internal/driver/), [`monarch/internal/controller/shadowtest_igris.go`](pipeline/monarch/internal/controller/shadowtest_igris.go).
...
**Kind E2E:** [`testing/scripts/e2e-reset-kind.sh`](testing/scripts/e2e-reset-kind.sh) applies `pipeline/siphon/deploy/rbac.yaml` only, patches `spec.siphon.image` from `$SIPHON_IMG`, and verifies Monarch pushed recorder config before tests. Do not fight Monarch with manual `kubectl set image` on the DaemonSet.

**Code:** [`siphon/internal/capture/`](pipeline/siphon/internal/capture/) (`bpf.go`, `capture.go`), [`siphon/internal/egress/`](pipeline/siphon/internal/egress/) (session, relay), [`siphon/internal/api/`](pipeline/siphon/internal/api/), [`monarch/internal/controller/shadowtest_siphon.go`](pipeline/monarch/internal/controller/shadowtest_siphon.go).
...
**Code:** [`recorder/`](recorder/), [`monarch/internal/controller/shadowtest_recorder.go`](pipeline/monarch/internal/controller/shadowtest_recorder.go).
...
See [`testing/scripts/manifests/e2e-shadowtest.yaml`](testing/scripts/manifests/e2e-shadowtest.yaml) and [`testing/scripts/e2e-reset-kind.sh`](testing/scripts/e2e-reset-kind.sh).
...
**Code:** [`beru/cmd/beru/main.go`](pipeline/beru/cmd/beru/main.go), [`beru/internal/envoyextproc/`](pipeline/beru/internal/envoyextproc/), [`beru/internal/ingest/`](pipeline/beru/internal/ingest/), [`beru/internal/api/http.go`](pipeline/beru/internal/api/http.go).
...
**E2E:** [`testing/scripts/e2e-egress-test.sh`](testing/scripts/e2e-egress-test.sh)
...
**E2E:** [`testing/scripts/e2e-record-replay.sh`](testing/scripts/e2e-record-replay.sh)
...
|-----------|-----------------|
| Monarch | `make -C pipeline/monarch deploy IMG=...` → `monarch-system` |
...
| Siphon | **Monarch-managed DaemonSet** in `siphon-system`; bootstrap RBAC via `kubectl apply -f pipeline/siphon/deploy/rbac.yaml`; image via `spec.siphon.image` |
...
**Kind E2E:** [`testing/scripts/e2e-reset-kind.sh`](testing/scripts/e2e-reset-kind.sh) — full reset/deploy, patches `$SIPHON_IMG` / `$IGRIS_IMG` on ShadowTest, verifies Siphon recorder config. Tests: [`testing/scripts/e2e-pipeline-test.sh`](testing/scripts/e2e-pipeline-test.sh) (ingress), [`testing/scripts/e2e-egress-test.sh`](testing/scripts/e2e-egress-test.sh) (4a.1), [`testing/scripts/e2e-record-replay.sh`](testing/scripts/e2e-record-replay.sh) (4a.2).
...
- [monarch/REPO_OVERVIEW.md](pipeline/monarch/REPO_OVERVIEW.md) — Monarch file layout
- [monarch/DEPLOYMENT.md](pipeline/monarch/DEPLOYMENT.md) — operator install

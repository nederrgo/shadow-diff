...
timestamp: 2026-07-28T18:45:00Z
---
...
A single `ShadowTest` CR drives the whole stack. Capture and analysis are **decoupled**: record mode is lightweight and always-on capable; replay mode spins A/B/C on demand against a pinned S3 session.
...
| [`pipeline/igrises/igris-rabbitmq/`](../../pipeline/igrises/igris-rabbitmq/) | AMQP fan-out (replay / hybrid ShadowTests) |
...
flowchart TB
  ...
  subgraph replayMode [Replay mode]
    ...
    ABC -->|ingress ext_proc| Beru[Beru]
...
└───────────────────────────────────┬─────────────────────────────────────────┘
                                    │
┌───────────────────────────────────▼─────────────────────────────────────────┐
│  L4  Analysis       Beru — ingress ext_proc + HTTP/AMQP/DB egress diffs     │
│                     Shop — Envoy egress ext_proc mock lookup + report       │
└─────────────────────────────────────────────────────────────────────────────┘

        ┌──────────────────────────────────────────────────────────┐
        │  Monarch — ShadowTest reconcile, mode GC, S3 secret sync,│
        │  auto replay start, retention finalizer                  │
        └──────────────────────────────────────────────────────────┘
```
...
| 4 | **Beru** | Ingress ext_proc + Shop async `POST /api/v1/egress/diff` → diff-of-diffs |
...
Shadow namespace layout (replay): three role Deployments (app + Envoy [+ shadow-soldier]), Shop, Igris, optional beru-local, per-role dependency Services.

---

## Analysis — Beru

Beru is the analysis sink for replay (and any path that reports diffs):

...
* [/data-plane/beru-bug-audit.md](/data-plane/beru-bug-audit.md) — Beru correctness audit: baseline signature buckets, WAL-on-accept, ingest idempotency, projection hard-fail; checkbox remediation tracker.
...
* [/data-plane/beru-postgres-storage.md](/data-plane/beru-postgres-storage.md) — Beru Postgres-only storage: Bbolt disk WAL with claimed parallel flushers, advisory-locked evaluate, `(replay_execution_id, ingest_id)` flush idempotency, UI projection tables, and `verdict_events` NOTIFY for Tusk.

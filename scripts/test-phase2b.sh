...
kubectl apply -f "$REPO/monarch/config/crd/bases/engine.shadow-diff.io_shadowtests.yaml" 2>/dev/null || make -C "$REPO/monarch" install
kubectl apply -f - <<EOF
...
kubectl rollout restart deployment/beru -n beru-system 2>/dev/null || kubectl apply -f "$REPO/beru/deploy/"
